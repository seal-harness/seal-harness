...
spec = describe "Seal.ISA.Ops.Memory" $ do
  describe "MEMORY_STORE" $ do
    it "stores an entry and returns 'stored'" $ do
      backend <- noneBackend
      let op = memoryStoreOp backend sampleSession
  ...
  describe "MEMORY_RECALL" $ do
    it "returns a paged window of all memories" $ do
      backend <- noneBackend
      -- store 3 memories
      let store op = opRun op localBackend
      _ <- runTestApp (store (memoryStoreOp backend sampleSession)
  ...
  describe "MEMORY_UPDATE" $ do
    it "updates an existing memory's content" $ do
      backend <- noneBackend
      _ <- runTestApp (opRun (memoryStoreOp backend sampleSession) localBackend
                             (object ["id" .= ("m1" :: Text), "content" .= ("old" :: Text)]))
      let update = memoryUpdateOp backend
      r <- runTestApp (opRun update localBackend (object ["id" .= ("m1" :: Text), "content" .= ("new" :: Text)]))
      orIsError r `shouldBe` False
      m <- mbRecall backend sampleMemoryId
      case m of
        Just e  -> meContent e `shouldBe` "new"
        Nothing -> expectationFailure "memory not found after update"

    it "errors when the memory does not exist" $ do
      backend <- noneBackend
      let update = memoryUpdateOp backend
      r <- runTestApp (opRun update localBackend (object ["id" .= ("nope" :: Text), "content" .= ("x" :: Text)]))
      orIsError r `shouldBe` True
