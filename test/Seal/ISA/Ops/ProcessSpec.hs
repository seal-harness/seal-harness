...
-- | A fake backend that records shell invocations and returns canned output.
...
spec = describe "Seal.ISA.Ops.Process" $ do
  ...
  describe "PROCESS_MANAGE" $ do
    ...
    it "list action runs a shell command and returns bounded output" $ do
      seen <- newIORef []
      let backend = fakeBackend seen "PID  CMD\n  1  init\n 42  myproc\n"
          op = processManageOp (WorkspaceRoot "/ws") (SecurityPolicy (AllowOnly mempty) Full) backend
      r <- runTestApp (uoRun op undefined backend (object ["action" .= ("list" :: String)]))
    ...
    it "kill action with a valid PID runs the kill command" $ do
      ...
      let backend = fakeBackend seen ""
          op = processManageOp (WorkspaceRoot "/ws") (SecurityPolicy (AllowOnly mempty) Full) backend
      r <- runTestApp (uoRun op undefined backend (object ["action" .= ("kill" :: String), "pid" .= (123 :: Int)]))
      orIsError r `shouldBe` False
      readIORef seen `shouldReturn` ["kill 123"]

    it "kill action rejects a negative PID" $ do
      let op = processManageOp (WorkspaceRoot "/ws") (SecurityPolicy (AllowOnly mempty) Full) undefined
    ...
    it "orRecorded captures the action + pid (secret-free)" $ do
      ...
      let backend = fakeBackend seen ""
          op = processManageOp (WorkspaceRoot "/ws") (SecurityPolicy (AllowOnly mempty) Full) backend
      r <- runTestApp (uoRun op undefined backend (object ["action" .= ("kill" :: String), "pid" .= (42 :: Int)]))
