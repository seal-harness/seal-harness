...
mkProbeOpcode ref = \case
  Trusted  -> TrustedOpcode (OpName "P") Trusted "p" (object []) (object [])
                            (const (Right ())) (\_ _ -> recordRunTrusted)
...
spec = describe "Seal.ISA.Dispatch" $ do
  it "Untrusted: ack precedes run" $ do
    ...
    let (h, op) = probe ref Untrusted
        reg = mkRegistry [op]
    _ <- runTestApp (dispatch reg h localBackend (mkTestUIOEnv testUntrustedIO stubCloneDeps) (OpName "P") (object []))
  ...
  it "missing opcode -> OpNotFound" $ do
    ...
    res <- runTestApp (dispatch (mkRegistry []) h localBackend (mkTestUIOEnv testUntrustedIO stubCloneDeps) (OpName "Z") (object []))
  ...
  it "failed authorization -> Denied, never runs" $ do
    ...
    res <- runTestApp (dispatch (mkRegistry [op]) h localBackend (mkTestUIOEnv testUntrustedIO stubCloneDeps) (OpName "P") (object []))
