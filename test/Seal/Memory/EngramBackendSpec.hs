...
-- engram backend correctly shells out to the engram CLI. They are guarded
-- with 'pendingWith' when the engram binary is not available.
...
spec = describe "Seal.Memory.EngramBackend" $ do
  describe "engramEmbeddingBackend" $ do
    it "indexes a file and search returns it as a result" $
      ...
      withSystemTempDirectory "seal-engram" $ \root -> do
        ...
        case mEngram of
          ...
          Just binPath -> do
            ...
            all ((/= filePath) . T.unpack . srPath) results `shouldBe` True
