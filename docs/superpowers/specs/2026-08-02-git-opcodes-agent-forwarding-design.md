...
**The key lifecycle (no un-encrypted secret on disk):**
1. **Generation** (at repo-create time, frontend-initiated, on the harness):
   `ssh-keygen -t ed25519 -f <harness-keyfile-path> -N "<vault-secret>"
   -C "seal-deploy-key:<repo-id>"` writes an **encrypted** private key
   (`aes256-ctr`/`bcrypt` — ciphertext) to `<harness-keyfile-path>` + a public
   key to `<harness-keyfile-path>.pub` under a 0700 harness-private dir
   (`~/.seal/state/repos/keys/`). The `<vault-secret>` (the passphrase) is
   generated by the harness (a random 32-byte value) and stored in the vault
   under `seal-deploy-key-passphrase:<repo-id>`. The **encrypted keyfile stays
   on the harness disk** (ciphertext — satisfies the no-*un-encrypted*-secret
   rule, same category as the age-encrypted vault file). The public key is
   stored as `srDeployKeyPublic` on the `SourceRepo` (public data, not secret)
   AND in `<harness-keyfile-path>.pub` (for re-display). No `ssh-keygen -f -`
   (which writes to a file named `-`); a real path under the harness private
   state dir. The passphrase is fed via `-N` (command-line) — note: argv
   exposure of the passphrase during generation is acceptable (the passphrase
   is random per-repo, not a user secret, and is rotated only by regenerating
   the key); alternatively the harness can pipe it to `ssh-keygen`'s stdin
   if `-N` is dropped (verify the stdin path during W5 implementation; `-N`
   is the fallback).
2. **Loading** (at git-op time, on the harness): the harness uses a
   **per-op ssh-agent** (see §4.6 for why per-op, not session-scoped): start
   `ssh-agent`, then `printf '<passphrase>\n' | SSH_ASKPASS_REQUIRE=never
   ssh-add <encrypted-keyfile>` — the passphrase is read from `vhGet` and
   piped to `ssh-add`'s **stdin** (verified: `ssh-add` reads the passphrase
   non-interactively from stdin with `SSH_ASKPASS_REQUIRE=never`; "Identity
   added"). `ssh-add` decrypts the encrypted keyfile (using the passphrase)
   into the agent's memory. The encrypted keyfile stays on disk (ciphertext);
   the cleartext key lives only in agent memory. `SSH_AUTH_SOCK` +
   `SSH_AGENT_PID` captured. **Immediately after the git op, `ssh-add -D`
   (delete all identities) + kill the agent.**
...
- The create flow is split into two endpoints (the designer's blocker — a
  ...
  1. `POST /api/repos` with `generate_key: true` → creates the `SourceRepo`
     (with `CredDeployKey { cVaultKey = "seal-deploy-key-passphrase:" <> repoId }`+
     a `srKeyfilePath` for the encrypted on-disk keyfile),
     generates the key (`ssh-keygen -t ed25519 -f <harness-keyfile-path> -N
     "<random-vault-passphrase>"`), stores the passphrase in the vault, keeps
     the encrypted keyfile on the harness disk, returns **201 + the `RepoInfo`
     descriptor** (no extra fields — the create response stays stable + typed).
...
- **Rotation** (use case 4): `POST /api/repos/:id/deploy-key/generate`
  regenerates the key (new `ssh-keygen` run → new encrypted keyfile overwriting
  the old; new passphrase in the vault under the SAME vault key name — the
  registry entry's `cVaultKey` is unchanged, satisfying S3), returns the new
  public key. The operator re-pastes at GitHub.
- `ReposView` renders the public key in a `<pre>` block with a "Copy" button
  ...
  (key NAME, never the private key). A faint hint next to the `pat`/
  `machine_user` credential dropdown option ("Token is presented to the
  untrusted machine; deploy keys are preferred.") surfaces the PAT residual
  in the web UX (matches the CLI help).
- **Repo-remove cleanup**: when a deploy-key repo is removed, the harness
  deletes the encrypted keyfile + `.pub` from the harness disk + deletes the
  passphrase vault entry. The encrypted keyfile is a per-repo artifact with a
  real lifecycle (create on register, delete on remove).
...
5. **W5 — Repo-create deploy-key generation + frontend** (`Seal.Gateway.API`
   ...
   **`ssh-keygen -t ed25519 -f <harness-keyfile-path> -N "<random-vault-passphrase>"`**
   (encrypted keyfile on the harness disk under `~/.seal/state/repos/keys/`;
   passphrase in the vault; public key on `srDeployKeyPublic` + the `.pub`
   file); host-aware instructions; the frontend "Generate" flow. Repo-remove
   deletes the encrypted keyfile + `.pub` + the passphrase vault entry.
   - RED: `ApiSpec` failing test — generate returns `RepoInfo`; `/deploy-key`
     ...
     private key in any response; the encrypted keyfile exists on the harness
     disk (ciphertext) + is deleted on repo-remove. `ReposView.test.tsx`
     failing test — the Generate flow renders the public key + Copy + instructions; private key
...
15. **`ssh-keygen -f <path> -N "<vault-secret>"` for keygen, encrypted keyfile
    on the harness disk** — the plan-gate feasibility review verified
    `ssh-keygen -f -` writes the private key to a file named `-` on disk
    (not stdout), and that NO existing Hackage library does the modern
    `openssh-key-v1` format for ed25519 (`crypto-pubkey-openssh` is 2015,
    RSA/DSA only, GHC 7.6-era). Rather than write a pure-Haskell OpenSSH
    serializer, the harness generates the key with `ssh-keygen` into a real
    **encrypted** keyfile (`-N "<random-vault-passphrase>"`) on the harness
    private state dir. The encrypted keyfile stays on the harness disk
    (ciphertext — satisfies the no-*un-encrypted*-secret rule, same category
    as the age-encrypted vault file); the passphrase lives in the vault
    (age-encrypted at rest); at git-op time `ssh-add` reads the passphrase
    from stdin (piped from `vhGet`) and decrypts the keyfile into agent
    memory. **No OpenSSH serializer to write; no `ssh-keygen -f -` disk
    issue; the biggest implementation risk is eliminated.** Verified:
    `printf '<pass>\n' | SSH_ASKPASS_REQUIRE=never ssh-add <enc-file>` works
    non-interactively ("Identity added"). The untrusted machine only sees
    the forwarded agent socket (encrypted keyfile never leaves the harness).
