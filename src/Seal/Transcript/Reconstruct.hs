...
-- invocations as distinct harness entries). The @input@, @result@, and
-- @channel@ fields are included when present in @erMeta@ (the 'SKILL_LOAD'
-- result entry recorded by 'Seal.ISA.Dispatch.recordSkillLoadResult' carries
-- input + result + the originating channel label, so the frontend can render
-- the skill body in a collapsible tool-call box AND surface channel
-- provenance in the source label). When the entry carries approval metadata
-- (erMeta has an "approval" key), the payload also includes the approval
-- info so the frontend can render the confirmation decision.
...
harnessPayload msgs e =
  let base = object
        ...
        , "input"    .= (Map.lookup "input" (erMeta e) :: Maybe Value)
        , "result"   .= (Map.lookup "result" (erMeta e) :: Maybe Value)
        , "channel"  .= (Map.lookup "channel" (erMeta e) :: Maybe Value)
        ]
