...
-- and @limit@. (A shared single-string schema helper cannot express optional
-- fields, so a small local builder is preferred over prematurely sharing one
-- here.)
...
--
-- 'orRecorded' captures the requested path / offset / limit (secret-free
-- metadata); file contents flow only to 'orParts' (model-visible).
...
fileReadOp root = Opcode
  ...
  , opRun = \backend v -> do
      ...
      -- Uniform orRecorded shape in both branches: path + offset + limit, all
      -- non-secret request metadata.
      let recorded = object ["path" .= rel, "offset" .= offset, "limit" .= mLimit]
      case mSafe of
        Left err ->
          pure $ OpResult
        ...
        Right safe -> do
          ...
          -- stays bounded (maxScanBytes).
          eWin <- runLocal backend $
            try @IOError (readLineWindow defaultPageParams offset mLimit safe)
          case eWin of
            Left ioErr ->
              pure $ OpResult
