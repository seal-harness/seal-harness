...
credOpt = T.pack <$> strOption
  ...
  <> help ( "Credential kind: pat (Personal Access Token: a token stored in "
         ...
         <> "process list); deploy_key (SSH deploy key: a private key stored "
         <> "in the vault, used to clone over SSH); machine_user (bot account: "
         <> "a username + token stored in the vault, used to clone over HTTPS "
         <> "as the bot user — requires --username)"
         )
  )
