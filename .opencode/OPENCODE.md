...
- `.coverage-thresholds.json` `enforcement.command` updated to `make test` (Haskell/cabal); HPC instrumentation for real line/branch/function/statement measurement is pending — until wired, the command runs the test suite but does not measure coverage
