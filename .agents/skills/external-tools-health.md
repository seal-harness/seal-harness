...
Check the status of external AI tools (Codex CLI, Gemini CLI) and their
configuration.
...
2. **Check configuration**: If a `docs/external-tools.yaml` file exists,
   read it and report which adapters are enabled/disabled.
...
4. **Report status**: Summary table showing each tool's install status,
